...
/*	for (CellCol=0; CellCol<HOG_CELL_SIZE; CellCol++) {
		for (CellLine=0; CellLine<HOG_CELL_SIZE; CellLine++)
			printf("%d ", In[CellCol*HOG_CELL_SIZE + CellLine]);
	printf("\n");
	}
	
	//static int num=0;
	for(int i=0;i<HOG_NBINS;i++) printf("%d ", Out[i]);
		printf("\n");*/
	//num++;
...

{
	...
	unsigned int CellCount = (W-PAD_W)/HOG_CELL_SIZE;
...
/*	
	for (Line=0; Line<HOG_BLOCK_SIZE; Line++){
		unsigned short* BlockLineVect = (CellLines[Line]+CellOffset);
			for (Col=0; Col<((HOG_BLOCK_SIZE*HOG_NBINS)); Col++) 
				printf("%d ",BlockLineVect[Col]);
			printf("\n");
	}
	printf("\n");
	*/
	...
	*/
	...
	for (Line=0; Line<HOG_BLOCK_SIZE; Line++) {
		...
		for (Col=0; Col<(HOG_BLOCK_SIZE*HOG_NBINS); Col++) {
			...
			//printf("%d ", (HOG_Features[(HOG_BLOCK_SIZE*HOG_NBINS)*Line+Col])  );
		}
	}
...

{
	...
	unsigned int CellW = (W-PAD_W)/HOG_CELL_SIZE;					// Number of cells in a line
