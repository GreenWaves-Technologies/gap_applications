...
#else
		    DataV[iB] = gap8_cplxmuls(Tmp, W) >> (v2s) {FFT2_SCALEDOWN, FFT2_SCALEDOWN};
#endif
...

{
	...
	// Y[n]=X[n]−0.95⋅X[n−1]
  	...
  	for (i=0; i<FrameSize; i++) {
		if (Active) {
		  Out[i] = (Frame[i]<<shift) - gap8_mulsRN(FP2FIX(0.97, Q15), S, 15);
			S = (Frame[i]<<shift);
		} else {
			Out[i] = Frame[i]<<Gain;
		}
...

{
	unsigned int i;
	  ...
	  OutFrame[i] = (v2s) {gap8_mulsRN(Frame[i], Window[i], 15), 0};
	}
...

{
	...
	*/
	...
	for (i=0; i<FilterBanks; i++) {
		...
		for (k=0, j=MFCC_FilterBank[i].Start; j<(MFCC_FilterBank[i].Start+Count); j++, k++) {
		  if (FramePower[j]>maxin) maxin =FramePower[j];
		}
		
		short int shift ;
		short int shift0 = gap8_fl1(maxin);
...
#ifdef PRINTDEB
		// check overflow
		short int shiftc = gap8_fl1(Coeff);
#endif
...
void Compute_dct(v2s *in_dct, v2s *twiddct, v2s *twiddles, short int *SwapLUT) {
  ...
  for (i=0;i<NDCT;i++) {
    v2s out_dct_loc = gap8_cplxmuls(in_dct[i],twiddct[i]);
